...
def clean_data(df):
    ...
    df["severity"] = df.get("severity", "").astype(str).str.title()
    df["diagnosis_code"] = df.get("diagnosis_code", "").astype(str).str.upper()
    df["payment_type"] = df.get("payment_type", "").astype(str).str.title()
...
def filter_dataframe(df):
    ...
    date_range = st.sidebar.date_input("Admission date range", [min_date.date(), max_date.date()], key="admission_date_range")
    df2 = df.copy()
    df2 = df2[(df2["admission_date"]>=pd.to_datetime(date_range[0])) & (df2["admission_date"]<=pd.to_datetime(date_range[1]))]
...
uploaded_file = st.sidebar.file_uploader("Upload dataset (ZIP with CSV, CSV or Excel)", type=["zip","csv","xlsx","xls"])
